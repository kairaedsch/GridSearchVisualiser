...
class AStar extends BasicSearchAlgorithm
{
   ...
   Node findNextActiveNode()
   {
      ...
      currentSearchState.description.add(new Explanation()
         ...
         ..addH("approximate the total distance", "yellow", new List.from(pathsOfOpen)..addAll(pathsOfOpenHeuristic)..addAll(open.map((on) => new TextHighlight((heuristic.calc(on, target) + getDistance(on).length()).toStringAsPrecision(2), on.position)).toList()))
