...
class AStar extends Dijkstra
{
   ...
   Node findNextActiveNode()
   {
      ...
      currentSearchState.description.add(new Explanation()
         ...
         ..addH("$heuristic", "yellow", new List.from(pathsOfOpenHeuristic)..addAll(open.map((on) => new TextHighlight(heuristic.calc(on, target).toStringAsPrecision(3), on.position)).toList()))
         ...
         ..addH("approximate the total distance", "yellow", new List.from(pathsOfOpen)..addAll(pathsOfOpenHeuristic)..addAll(open.map((on) => new TextHighlight((heuristic.calc(on, target) + getDistance(on).length()).toStringAsPrecision(3), on.position)).toList()))
         ...
         ..addH("shortest approximated total distance", "green", [new PathHighlight(getPath(nStar).map((n) => n.position).toList(), showEnd: true), new PathHighlight.styled("yellow dotted", heuristic.getPath(nStar, target), showEnd: true), new TextHighlight((heuristic.calc(nStar, target) + getDistance(nStar).length()).toStringAsPrecision(3), nStar.position)])
